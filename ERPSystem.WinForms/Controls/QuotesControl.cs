...
public class QuotesControl : UserControl, IRealtimeDataControl
{
    ...
    public QuotesControl(QuoteRepository quoteRepository, ProductionRepository productionRepository, UserManagementRepository userRepository, UserAccount currentUser, Action<string> openSection, StepParsingDiagnosticsLog? stepParsingDiagnosticsLog = null)
    {
        ...
        _completedQuotesGrid.CellDoubleClick += async (_, e) => await OpenCompletedQuoteDetailsAsync(e.RowIndex);
    ...
    private async Task HandleCompletedQuoteCellClickAsync(int rowIndex, int _)
    {
        ...
        await OpenCompletedQuoteDetailsAsync(rowIndex);
    }

    private async Task OpenCompletedQuoteDetailsAsync(int rowIndex)
    {
        if (rowIndex < 0 || rowIndex >= _completedQuotesGrid.Rows.Count)
        {
            return;
        ...
        {
            _feedback.Text = $"Completed quote {quoteId} could not be loaded.";
            return;
        }
    ...
    private Control CreateModelCell(QuoteLineItem lineItem)
    {
        ...
        var viewer = new StepModelPreviewControl(_stepParsingDiagnosticsLog)
        {
            Dock = DockStyle.Fill,
